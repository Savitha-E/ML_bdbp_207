...
#Convergence
def identify_convergence(cost_func,theta,X,y,iterations):
    cost_fun_list=[]
    for i in range(iterations):
        theta_val = gradient_function(updated_theta, theta, X, y)
        theta = theta_val
        print("This is is updated theta values:", theta, "and this is the cost function for it",
              cost_function(theta, X, y))
        z=cost_func(theta, X, y)
        cost_fun_list.append(z)
    return cost_fun_list

convergence=identify_convergence(cost_func,theta,X,y,iterations)
print(convergence)
